...
- **Category field:** tenant config names the ticket field (`desk.category_field`, e.g. `"category"`); its picklist values are fetched via `GET /organizationFields?module=tickets` and cached at boot. Ticket subject = the chosen category value; there's no separate free-text subject field — confirmed against the shipped app's create-ticket screen, which only collects category + an optional description.
